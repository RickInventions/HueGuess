 ...
 *
 * Mirrored verbatim in `frontend/src/lib/ranks.ts` — the two packages share no
 * build, so if you change a band here, change it there too.
 *
...
export function getRankDivision(rating: number): RankDivision {
  ...
  return { tier: band.tier, division, label: `${band.tier} ${ROMAN[0]}`, ...bounds, isMax: isTopBand };
}

/** Display label stored in `competitive_stats.rank_tier`, e.g. "Gold II". */
